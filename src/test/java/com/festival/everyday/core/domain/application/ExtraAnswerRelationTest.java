...
class ExtraAnswerRelationTest {
    ...
    @Test
    @DisplayName("지원서에 추가 답변을 작성합니다.")
    void createWithApplication() {
        List<ExtraAnswer> extraAnswers = ExtraAnswer.createExtraAnswers(application, str);
        Assertions.assertThat(extraAnswers.getFirst().getApplication()).isEqualTo(application);
        Assertions.assertThat(extraAnswers.getFirst().getApplication().getExtraAnswers().containsAll(extraAnswers)).isTrue();
    }
}
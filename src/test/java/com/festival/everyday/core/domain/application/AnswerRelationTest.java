...
class AnswerRelationTest {
    ...
    @Test
    @DisplayName("지원서에 답변을 작성합니다.")
    void createWithApplication() {
        List<Answer> answers = Answer.createAnswers(application, str);
        Assertions.assertThat(answers.getFirst().getApplication()).isEqualTo(application);
        Assertions.assertThat(answers.getFirst().getApplication().getAnswers().containsAll(answers)).isTrue();
    }

...
public class FestivalMapper {
    ...
    public static Festival of(Holder holder, FestivalFormRequest request) {
        return Festival.create(holder, request.getName(),
                Period.create(request.getPeriod().getBegin(), request.getPeriod().getEnd()),
                request.getFee(), request.getTime(), request.getIntroduction()
                , request.getLink(), request.getTel(),
                Address.create(request.getAddress().getCity(), request.getAddress().getDistrict(), request.getAddress().getDetail()));
    }
}

...
//    private final RecruitQueryService recruitQueryService;
    ...
    public ResponseEntity<ApiResponse<Long>> createCompanyRecruit(@RequestBody CreateCompanyRecruitRequest request) {
        return ResponseEntity.ok(ApiResponse.success("업체 모집 공고 등록에 성공했습니다.", recruitCommandService.saveCompanyRecruit(request)));
    }
    ...
    public ResponseEntity<ApiResponse<Long>> createLaborRecruit(@RequestBody CreateLaborRecruitRequest request) {
        return ResponseEntity.ok(ApiResponse.success("근로자 모집 공고 등록에 성공했습니다.", recruitCommandService.saveLaborRecruit(request)));
    }
